...
const mockPasswordResetTokenStorageUpdate = prisma.passwordResetTokenStorage.update as Mock;
...
describe('resetPasswordService', () => {
  it('should reset the password correctly', async () => {
    ...
    const result = await resetPasswordService(mockUser.id, 'reset-token', mockUser.password);

    expect(mockPasswordResetTokenStorageUpdate).toHaveBeenCalled();
  ...
  it('should throw AppError with 400 if the userID is an empty string', async () => {
    await expect(resetPasswordService('', 'reset-token', mockUser.password)).rejects.toMatchObject({
  ...
  it('should throw AppError with 401 if the reset token is null', async () => {
    await expect(resetPasswordService(mockUser.id, '', mockUser.password)).rejects.toMatchObject({
