...
const mockPasswordResetTokenStorage = prisma.passwordResetTokenStorage.create as Mock;
...
describe('forgotPasswordService', () => {
  it('should send the e-mail to recover password correctly', async () => {
    ...
    expect(result.message).toBe('Recovery e-mail successfully sent.');
  });

  it('should throw AppError with 404 if the logIn user does not exist', async () => {
    mockFindUnique.mockResolvedValue(null);

    await expect(forgotPasswordService(mockUser.email)).rejects.toThrowError(AppError);
    await expect(forgotPasswordService(mockUser.email)).rejects.toMatchObject({
      message: "There's no active user with this e-mail address.",
      status: 404,
    });
  });
});
