...
export async function updateGiftService(
  ...
  if (!gift) {
    throw new AppError("Couldn't find this gift on the database.", 404);
  }
...
export async function deleteGiftService(userID: string, giftID: number) {
  const user = await prisma.users.findUnique({
    where: {
      id: userID,
    },
  ...
  if (!gift) {
    throw new AppError("Couldn't find the gift on the database.", 404);
  }
...
export async function createGiftService(
  ...
  const existentNames = new Set(existentGifts.map((gift) => gift.productName.trim().toLowerCase()));
...
export async function getGiftsService(userID: string, weddingID: number) {
  const user = await prisma.users.findUnique({
    where: {
      id: userID,
    },
  ...
  const checkCreatorArray = userPart?.weddingsOwn.filter((weddings) => weddings.id === weddingID);
...
export async function getGiftedProductsService(userID: string, weddingID: number) {
  const user = await prisma.users.findUnique({
    where: {
      id: userID,
    },
  ...
  if (checkWedding.createdBy !== userID) {
    throw new AppError("You don't have access to this page.", 403);
  }
