...
export async function updateGiftController(req: CustomRequest, res: Response) {
  const { id } = req.authUser!;
  const { giftID, productLink, productName, quantity } = req.body;
...
export async function giftPresentController(req: CustomRequest, res: Response) {
  const userID = req.authUser!.id;
  const { id } = req.query;
  const { giftID, quantity } = req.body;

  try {
    const response = await giftPresentService(userID, Number(id), Number(giftID), Number(quantity));
    const message = response.message;
    res.status(200).json({ message });
  } catch (error) {
    controllerErrorHandler(error, res);
...
export async function deleteGiftController(req: CustomRequest, res: Response) {
  const userID = req.authUser!.id;
  const { giftID } = req.body;

  try {
    const response = await deleteGiftService(userID, Number(giftID));
    const message = response.message;
    res.status(200).json({ message });
  } catch (error) {
    controllerErrorHandler(error, res);
...
export async function createGiftController(req: CustomRequest, res: Response) {
  const userID = req.authUser!.id;
  const weddingID = Number(req.query.id);
  const newGiftsArray: giftProps[] = req.body;

  try {
    const response = await createGiftService(userID, weddingID, newGiftsArray);
    const message = response.message;
...
export async function getGiftsController(req: CustomRequest, res: Response) {
  const userID = req.authUser!.id;
  const weddingID = Number(req.query.id);
...
export async function getGiftedProductsController(req: CustomRequest, res: Response) {
  const userID = req.authUser!.id;
  const weddingID = Number(req.query.id);
