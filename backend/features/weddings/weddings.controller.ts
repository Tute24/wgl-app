...
export async function createWeddingController(req: CustomRequest, res: Response) {
  ...
  try {
    ...
    const newlyCreatedWedding = response.newlyCreatedWedding as WeddingsProps;
...
export async function getWeddingsController(req: CustomRequest, res: Response) {
  const userID = req.authUser!.id;
...
export async function deleteWeddingController(req: CustomRequest, res: Response) {
  const userID = req.authUser!.id;
  const weddingID = Number(req.body.id);

  try {
    const response = await deleteWeddingService(userID, weddingID);
    const message = response.message;
