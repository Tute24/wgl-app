...
describe('WeddingCard', () => {
  ...
  function renderComponent(isOwn: boolean) {
    return render(
      <WeddingCard
        ...
        title={
          isOwn ? weddingsMock.ownWeddings.weddingTitle : weddingsMock.guestWeddings.weddingTitle
        }
        date={isOwn ? weddingsMock.ownWeddings.weddingDate : weddingsMock.guestWeddings.weddingDate}
        isOwn={isOwn}
  ...
  it('should render owner card correctly', () => {
    ...
    expect(screen.getByText(weddingsMock.ownWeddings.weddingDate)).toBeInTheDocument();
    expect(screen.getByText(weddingsMock.ownWeddings.weddingTitle)).toBeInTheDocument();

    const ctaLinkText = screen.getByRole('link', {
      name: /go to this wedding's page/i,
    });
    fireEvent.click(ctaLinkText);
    expect(ctaLinkText).toHaveAttribute('href', `/portal/giftList/${weddingsMock.ownWeddings.id}`);
  });

  it('should render guest card correctly', () => {
    renderComponent(false);

    expect(screen.getByText(weddingsMock.guestWeddings.weddingDate)).toBeInTheDocument();
    expect(screen.getByText(weddingsMock.guestWeddings.weddingTitle)).toBeInTheDocument();
