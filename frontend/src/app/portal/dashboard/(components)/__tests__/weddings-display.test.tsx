...
describe('WeddingsDisplay', () => {
  ...
  it('should show no weddings message if there are no weddings to exhibit', () => {
    ...
    expect(screen.getByText('There are no weddings to exhibit at this section.')).toBeVisible();
  });
  ...
  it('should show display the wedding card if ownWeddings is not an empty array', () => {
    ...
    expect(screen.getByTestId(`wedding-${weddingsMock.ownWeddings.id}`)).toBeInTheDocument();
    expect(screen.getByText(weddingsMock.ownWeddings.weddingTitle)).toBeVisible();
  });
});
