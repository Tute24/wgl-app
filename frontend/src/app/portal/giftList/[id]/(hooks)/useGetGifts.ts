...
export default function useGetGifts() {
  ...
  const { setWeddingGifts, setIsCreator, setListHeader, setIsGuest } = useGiftsStore(
    useShallow((store) => ({
  ...
  async function getGifts() {
    try {
      ...
      if (response.status === 200) {
        setIsCreator(response.data.responseObject.checkPreferences.isCreator as boolean);
        setIsGuest(response.data.responseObject.checkPreferences.isGuest as boolean);
        setListHeader(response.data.responseObject.listHeader as listHeaderProps);
        setWeddingGifts(response.data.responseObject.weddingGifts as weddingGiftsProps[]);
      }
    } catch (error) {
      AxiosErrorHandler({
        error,
