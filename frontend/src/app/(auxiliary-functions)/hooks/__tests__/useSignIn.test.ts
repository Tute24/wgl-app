...
describe('useSignIn', () => {
  it('should call the useSignIn hook successfully', async () => {
    ...
    expect(mockAxiosInstancePost).toHaveBeenCalledWith('/auth/sign-in', mockData);
  ...
  it('handles 401 error', async () => {
    ...
    expect(mockAxiosInstancePost).toHaveBeenCalledWith('/auth/sign-in', mockData);
    expect(mockSetStatusMessage).toHaveBeenCalledWith(`Incorrect password. Try again!`);
  });
  ...
  it('handles 500 error', async () => {
    ...
    expect(mockAxiosInstancePost).toHaveBeenCalledWith('/auth/sign-in', mockData);
    expect(mockSetStatusMessage).toHaveBeenCalledWith(`Something went wrong. Try again!`);
  });
});
