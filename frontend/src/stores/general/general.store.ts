...
export const createGeneralStore = (initState: GeneralStoreState = defaultInitState) => {
  return createStore(
    persist<GeneralStore>(
      (set) => ({
        ...initState,
        setIsLoading: (isLoading) => set((store) => ({ ...store, isLoading })),
        setIsRendering: (isRendering) => set((store) => ({ ...store, isRendering })),
        setStatusMessage: (statusMessage) => set((store) => ({ ...store, statusMessage })),
        setPendingRequests: (pendingRequests) => set((store) => ({ ...store, pendingRequests })),
        setUsername: (username) => set((store) => ({ ...store, username })),
        setModalObject: (modalObject) => set((store) => ({ ...store, modalObject })),
